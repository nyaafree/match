<template>
    <div>
        <h1 class="index-title" id="register">登録案件一覧</h1>
         <a :href="'items/' + item.id" class="c-panel" v-for="(item, index) in receiveMyItems" :key="item.id" >
        ...
        <div class="c-panel__boards">
            ...
            <a :href="'board/' + board.id " v-for="board in item.boards" :key="board.id" style="display:block;">掲示板番号{{ board.id }}
             応募ユーザー: {{ board.apply.user.name }}</a>
        </div>
        <a :href="'items/' + item.id + '/edit' "><button class="btn btn-yellow">編集</button></a>
        <form :action="'items/' + item.id " style="display:inline" method="POST">
